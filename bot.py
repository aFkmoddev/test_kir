...
def hasAds(msg):
	links = ["http://","https://",".ir",".com",".org",".net",".me","/",".","@","!","!help"]
	for i in links:
		if i in msg:
			return True
...
def fal_hafez(text,chat,bot):
    try:
        res = requests.get('https://api.codebazan.ir/fal/')
        if res.status_code == 200 and res.content != b'':
            b2 = res.content
            width, height = bot.getImageSize(b2)
            tx = bot.requestFile('jok_'+ str(random.randint(1000000, 9999999)) + '.png', len(b2), 'png')
...
def get_kind(text,chat,bot):
    try:                        
        jd = requests.get('https://api.codebazan.ir/khabar/index.php?kind=iran').text
        bot.sendMessage(chat['object_guid'], jd, chat['last_message']['message_id'])
    except:
        print('code bz kind err')
    return True

def get_dialog(text,chat,bot):
    try:                        
        jd = requests.get('http://api.codebazan.ir/dialog/').text
        bot.sendMessage(chat['object_guid'], jd, chat['last_message']['message_id'])
    except:
...
def get_hadis(text,chat,bot):
    try:                        
        jd = requests.get('http://api.codebazan.ir/hadis/').text
        bot.sendMessage(chat['object_guid'], jd, chat['last_message']['message_id'])
    except:
...
def get_added(text,chat,bot):    
    try:
        ...
        send_text = 'سلام کابر گرامی ' + group + ' خوش اومدی ⁉️\n لطفا قوانین گروهو رعایت کن !'
        bot.sendMessage(chat['object_guid'],  send_text, chat['last_message']['message_id'])
    except:
        print('rub server err')
...
def get_addgap(text,chat,bot):                                
    text = open('addgap.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
        bot.sendMessage(chat['object_guid'], text, chat['last_message']['message_id'])
    print('addgap guid sended')

def get_ghavanint(text,chat,bot):                                
    text = open('ghavanint.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
        bot.sendMessage(chat['object_guid'], text, chat['last_message']['message_id'])
    print('ghavanint guid sended')
    
def get_Sandn(text,chat,bot):                                
    text = open('Sandn.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
        bot.sendMessage(chat['object_guid'], text, chat['last_message']['message_id'])
    print('Sandn guid sended')
    
def get_pol(text,chat,bot):                              
    text = open('pol.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
        bot.sendMessage(chat['object_guid'], text, chat['last_message']['message_id'])
    print('pol guid sended')
    
def get_gogel(text,chat,bot):                                
    text = open('gogel.txt','r').read()
    if chat['abs_object']['type'] == 'Group':
        bot.sendMessage(chat['object_guid'], 'نتایج کامل به پیوی شما ارسال شد', chat['last_message']['message_id'])
        bot.sendMessage(chat['last_message']['author_object_guid'], text)                                        
    elif chat['abs_object']['type'] == 'User':
        bot.sendMessage(chat['object_guid'], text, chat['last_message']['message_id'])
    print('gogel guid sended')
                    ...
                    if 'SendMessages' in access and chat['last_message']['type'] == 'Text' and text.strip() != '':                    
                        ...
                        if not m_id in list_message_seened:
                            ...
                            if chat['abs_object']['type'] == 'User' and not chat['object_guid'] in qrozAdmins and open('your_channel.txt','r').read() != '' and not chat['object_guid'] in black_users:
                                if not chat['object_guid'] in verfied_users and text.startswith('!') or text.startswith('سلام') or text.startswith('ربات') or text.startswith('.') or text.startswith('سلم') or text.startswith('صلم') or text.startswith('سيلام') or text.startswith('سلام'):
                                    is_joined = bot.checkJoinChannel(chat['object_guid'], open('your_channel.txt','r').read())
                                    if is_joined == 'no exist':
                                        accept = False
                                        jok = datetime.datetime.now() 
                                        bot.sendMessage(chat['object_guid'], '(ســـلام کــاربــر عـــزیــز) 🌺\n\nجــهت فــعال شـــدن رــبات در چـــنل زـیر ( جـــویـــن شوید )\n{ @yorz_bit }\nو اســـتارت کــنید 😉\n!start\nموفــق بـــاشـــید')
                            ...
                            if accept == True:        
                                if text == '!start':
                                    ...
                                    try:
                                        bot.sendMessage(chat['object_guid'], ' (🅿️) فعال هستم راهنما \n  !help',chat['last_message']['message_id'])
                                        print('sended response')    
                                    except:
                                        print('server bug1')
                                    ...
                                    tawd30 = Thread(target=get_dialog, args=(text, chat, bot,))
                                    tawd30.start()
                               
                                elif text.startswith('/hadis'):
                                    tawd30 = Thread(target=get_hadis, args=(text, chat, bot,))
                                    tawd30.start()
                                elif text.startswith('/zekr'):
                                    tawd30 = Thread(target=get_zekr, args=(text, chat, bot,))
                                    tawd30.start()
                                elif text.startswith('/news'):
                                    tawd30 = Thread(target=get_kind, args=(text, chat, bot,))
                                    tawd30.start()
                                    ...
                                    tawd27 = Thread(target=get_email, args=(text, chat, bot,))
                                    tawd27.start()
                                    ...
                                    tawd12 = Thread(target=fal_hafez, args=(text, chat, bot,))
                                    tawd12.start()
                                    ...
                                    tawd38.start()        
                                     ...
                                     tawd38 = Thread(target=get_gogel, args=(text, chat, bot,))
                                     tawd38.start()
                                elif text.startswith('/Entertainment'):
                                    tawd38 = Thread(target=get_Sandn, args=(text, chat, bot,))
                                    tawd38.start()
                                elif text.startswith('/addgap'):
                                    tawd38 = Thread(target=get_addgap, args=(text, chat, bot,))
                                    tawd38.start()
                                
                                elif text.startswith('/ghavanint'):
                                    tawd38 = Thread(target=get_ghavanint, args=(text, chat, bot,))
                                    tawd38.start()           
                                elif text.startswith('/google'):
                                    tawd38 = Thread(target=get_pol, args=(text, chat, bot,))
                                    tawd38.start()
                                    ...
                                    try:
                                        ...
                                        if len(guid) == 32 and not guid in black_users:
                                            ...
                                            fp.write('\n' + guid)
                                            fp.close()
                                            bot.sendMessage(guid, 'متاسفانه شما فعلا به لیست سیاه  اضافه شدید')
                                    except:
                                    ...
                                    try:
                                        ...
                                        if len(guid) == 32 and guid in black_users:
                                            ...
                                            bl.close()
                                            bot.sendMessage(guid, 'خوشبختانه از لیست سیاه  بیرون آمدید')
                                    except:
                                        print('black list err')
                                    ...
                                    fp.write('\n'+ chat['object_guid'])
                                    fp.close()
                                    bot.sendMessage(chat['object_guid'], 'گروه به لیست سیاه اضافه شد 🗑')
                                elif chat['object_guid'] == g_usvl and chat['last_message']['author_object_guid'] != 'u0DiUsx030cf57263cf01e33f65a3614' and chat['abs_object']['type'] == 'Group' and not text.startswith('!'):
                                    tawd42 = Thread(target=usvl_save_data, args=(text, chat, bot, auths,))
                                    tawd42.start()
                                elif test_usvl == chat['object_guid'] and chat['last_message']['author_object_guid'] != 'u0DiUsx030cf57263cf01e33f65a3614' and chat['abs_object']['type'] == 'Group' and not text.startswith('!'):
                                    print('usvl tested')
                                ...
                                bl.write(black_users.replace('\n' + chat['object_guid'], ''))
                                bl.close()
                                bot.sendMessage(chat['object_guid'], 'گروه از لیست سیاه در اومد✅')
